...
export default function FullHome() {
  ...
  useEffect(() => {
    supabase.from('site_banners').select('image_url').eq('is_active', true).order('sort_order', { ascending: true }).then(({ data }) => {
      const urls = (data || []).map(b => b.image_url)
                ...
                {banners.length > 0 ? (
                  banners.map((url, i) => (
                    <img
                      ...
                      src={url}
                      alt=""
                      className={`relative rounded-2xl shadow-2xl w-full h-auto object-cover aspect-[4/3] transition-opacity duration-700 ${i === currentBanner ? 'block' : 'hidden'}`}
                    />
                  ))
                ) : (
                  <img
