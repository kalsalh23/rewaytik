...
import { Copy, Check, Wallet, CreditCard, ArrowLeft, Upload, X, Loader2, Image as ImageIcon } from 'lucide-react'
import { Button } from '@/components/ui/Button'
import { Card, CardContent } from '@/components/ui/Card'
import { toast } from 'react-hot-toast'
import { supabase } from '@/lib/supabase'
import { getManuscriptOrder, updateManuscriptPayment } from '@/lib/supabase-service'
...
export default function ManuscriptPayment() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateManuscriptPayment(id, publicUrl, shamCashInfo.walletNumber)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-20 px-4 bg-gradient-to-b from-primary/5 to-background">
      <div className="w-full max-w-lg">
        <button onClick={() => navigate('/my-manuscripts')} className="inline-flex items-center gap-2 text-sm text-secondary/60 hover:text-primary mb-6 transition-colors cursor-pointer">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-secondary-light mt-2">أكمل عملية الدفع لتأكيد طلب اصنع كتابك</p>
        </div>
          ...
          {order && (
            <div className="p-4 rounded-xl bg-accent/30 mb-6">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-secondary/60">الكتاب</span>
          ...
          {submitting && (
            <div className="mt-6 p-4 rounded-xl bg-primary/5 border border-primary/20">
              ...
              <div className="w-full h-3 bg-border rounded-full overflow-hidden">
                <div className="h-full gradient-primary rounded-full transition-all duration-300 ease-out" style={{ width: `${uploadProgress}%` }} />
              </div>
            </div>
          )}
